...
for exp in exp_list:
    for dset in dset_list:
        mat = np.load('./iden_all_confusion_mats/exp' + exp + '/'+dset+'_mat.npy')
        fmt = '.2f' 
        thresh = mat.max() / 2.
        top = mpl.cm.get_cmap('Reds', 128)
        ...
        plt.ylabel('labels')
        plt.xlabel('labels')
        plt.savefig('./iden_all_confusion_mats/exp'+exp + '/' + dset+'_confusion_mat.jpg')
        plt.close()
